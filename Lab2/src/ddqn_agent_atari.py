...
class AtariDQNAgent(DQNBaseAgent):
	...
	def update_behavior_network(self):
		...
		with torch.no_grad():
			q_next_with_behavior = self.behavior_net(next_state)
			argmax_a_on_behavior = q_next_with_behavior.argmax(dim=1).unsqueeze(1)
   
			q_next_with_target = self.target_net(next_state)
			q_next = q_next_with_target.gather(1, argmax_a_on_behavior)

...
public:
	...
	virtual float estimate(const board& b) const {
		...
		2. for each isomorphic[i]
			get the idx of 1 with [0, 0, ..., 1, ..., 0, 0] from b
			value += weight[index];
		3. return the value
		*/
		float value = 0;
	...
	virtual float update(const board& b, float u) {
		// TODO
		/*
		u = alpha(0.1) * prdiction error
		1. u /= iso_last(step size)
		2. initialize value = 0
		3. for each isomorphic[i]
			get the idx of 1 with [0, 0, ..., 1, ..., 0, 0] from b
			update weight by using weight[index] += u
		4. return the new value after updated
		*/
		u /= iso_last;
		float value = 0;
		for(int i = 0; i < iso_last; i++){
...
protected:
	...
	size_t indexof(const std::vector<int>& patt, const board& b) const {
		// TODO
		/*
		get idx of 1 with [0, 0, ..., 1, ..., 0, 0] from b
		1. initialize index = 0
		2. for each patt[i]
			shifting the index to left by 4 bits (e.g. 00001111 => 11110000)
			index += b.at(patt[i]) (e.g. 11110000 + 00000101 => 11110101)
		3. return the index
		*/
		size_t index = 0;
	...
	state select_best_move(const board& b) const {
		...
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) {
				// TODO
				/*
					1. for each s'(s after move)
						enumeate all possible s''(s' after popup)
						summing up the value with
							0.9 * s'' value(popup 2) + 0.1 * s'' value(popup is 4)
						average the value by the number of empty tiles
					2. choose the best move with the highest s'' value
				*/
				int cnt_zero = 0;
				...
				for(int i = 0; i < 16; i++){
					if(tmp.at(i) == 0){
						cnt_zero++;
						tmp.set(i, 1);
						all_possible_value += 0.9 * estimate(tmp);
						tmp.set(i, 2);
						all_possible_value += 0.1 * estimate(tmp);
						tmp.set(i, 0);
					}
				}
				if(cnt_zero != 0){

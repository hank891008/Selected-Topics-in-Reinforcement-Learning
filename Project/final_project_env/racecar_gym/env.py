...
class RaceEnv(gym.Env):
    ...
    def step(self, actions):
        ...
        reward = 0
        truncated |= (state['time'] >= 25)
            
        if state['wall_collision'] == True:
            reward -= 100
        
        if state['progress'] >= (1 - 1e-3):
            reward += 100
        ...
        if state['progress'] > self.prev_info['state']['progress']:
            reward += 1000 * (state['progress'] - self.prev_info['state']['progress'])
        elif state['progress'] <= self.prev_info['state']['progress']:
            reward -= 0.1
        
        if state['wrong_way'] == True:
            reward = -reward
